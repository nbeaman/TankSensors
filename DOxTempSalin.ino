...
void loop() {
  ...
  if (((millis() - SensorAutoReadingMillis) > 2000) && !GV_WEB_REQUEST_IN_PROGRESS){
    ...
    if (CODE_FOR_TEMPSALINITY_DEVICE){
      ...
      LCD_DISPLAY(String(GV_TEMP) + "  ", 0, 1, NoClearLCD, PrintSerial);
      SendCommandToSensorAndSetReturnGVVariables("100:r");
      LCD_DISPLAY(String(GV_SALIN) + " ", 16-GV_SENSOR_DATA.length(), 1, NoClearLCD, PrintSerial);
    }
...
void SendCommandToSensorAndSetReturnGVVariables( String command ) {
  ...
  if( CODE_FOR_DOx_DEVICE ){
    GV_DOX                = GV_SENSOR_DATA.toFloat();
    GV_WEB_RESPONSE_TEXT  = GV_SENSOR_DATA + ":" + GV_SENSOR_RESPONSE;
  }
  if( CODE_FOR_TEMPSALINITY_DEVICE && channel==SalinitySensorAddress ){
    GV_SALIN              = GV_SENSOR_DATA.toFloat();
    GV_WEB_RESPONSE_TEXT  = GV_SENSOR_DATA + "," + String(GV_TEMP) + ":" + GV_SENSOR_RESPONSE;   //convert back to string ROUNDS the number to nearest 100th.
  }
  if( CODE_FOR_TEMPSALINITY_DEVICE && channel==TemperatureSensorAddress ){
    GV_TEMP               = GV_SENSOR_DATA.toFloat();
    GV_WEB_RESPONSE_TEXT  = String(GV_SALIN) + "," + GV_SENSOR_DATA + ":" + GV_SENSOR_RESPONSE;  //convert back to string ROUNDS the number to nearest 100th.
  }
